...
class InfiniteBalancedBatchSampler(Sampler):
    ...
    def __init__(self,
        ...
        self.ratio = ratio
        # assert self.batch_size % (self.ratio[0] + self.ratio[1]) == 0
    ...
    def __iter__(self):
        ...
        for idx in self.indices:
            # if len(batch_buffer) >= self.batch_size - 1:
            # if len(batch_buffer) >= self.batch_size:  # first intra training no inter data used
                # idx = np.random.randint(self.cross_range[-2],self.cross_range[-1])
